...
def JeibaCutWords(input_df, target, label='Class', id='ID'):
...
def PlotWordCloud(words_source):
    ...
    font = r'/content/suicide_multiclass/mingliu.ttc'
    #想要文字雲出現的圖示
...
def ShowWordCloud(input_df):
    low_terms = list([])  # 儲存低風險的單詞
    medium_terms = list([])  # 儲存中風險的單詞
    high_terms = list([])  # 儲存高風險的單詞

    for index, row in input_df.iterrows():
        #print(row['sentence'])
        if row['class'] == 0:  #
            for word in row['sentence']:
                low_terms.append(word)  # 收集低風險詞
        if row['class'] == 1:  #
            for word in row['sentence']:
                medium_terms.append(word)  # 收集中風險詞
        if row['class'] == 2:  #
            for word in row['sentence']:
                high_terms.append(word)  # 收集高風險詞
    print("低風險文字雲:")
    PlotWordCloud(low_terms)
    print("中風險文字雲:")
    PlotWordCloud(medium_terms)
    print("高風險文字雲:")
    PlotWordCloud(high_terms)


def process_df(article_df):
  #turn sentence and label into [['甲 乙 丙 丁', label], [,]....]
  corpus = []
  for i in range(len(article_df)):
    temp = []
    temp.append(' '.join(article_df.loc[i, 'sentence']))
    temp.append(article_df.loc[i, 'class'])
    corpus.append(temp)
